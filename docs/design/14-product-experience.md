...
PumpkinPi is the unified place where one person works with Projects through **Intent Chat**. Each Project has a **Source of Intent** that PumpkinPi implements on the Spoke where the real context lives, iterating situated implementation, validation, and independent whole-Project review until the reviewer finds no fault.
...
2. **Where work is happening**: spoke, project path, execution user, and trust boundary.
...
- **Personal Hub**: the unified view of the owner's Projects, Spokes, preferences, and recent activity.
- **Spoke**: one connected computer contributing real execution context. Spoke surfaces expose online/offline/revoked state, version, hostname, and last seen time.
- **Project**: a trusted working directory on a Spoke, defined by a Source of Intent and primarily accessed through Intent Chat.
...
3. Show enrolled Spokes or explain that none are enrolled.
4. Provide a Spoke enrollment path.
5. Select a Spoke and choose a directory.
...
1. Open PumpkinPi.
2. See recent Projects and active Intent Chats across all Spokes.
3. Resume a Project.
4. Read what PumpkinPi believes the current intent/status is.
...
8. Let PumpkinPi iterate implementation, validation, and independent whole-Project review internally.
...
- Spoke offline: show last known Project state and Intent Chat, mark live execution unavailable, offer retry diagnostics.
...
- Client reconnect: resubscribe to previous Project/Intent Chat state and replay durable timeline entries after the last known cursor.
...
- Left pane: recent Projects, grouped or filterable by Spoke, with status badges.
- Center pane: selected Project's Intent Chat.
- Right pane: contextual inspector: spoke/cwd/effective user/model/provider, current status, evidence, changed files, running internal work, diagnostics, raw event toggle.
...
- Spoke: online, offline, disabled, revoked, version-mismatch.
...
PumpkinPi stops feeling like a prototype when:
...
- PumpkinPi continues implementing reviewer findings until independent whole-Project review finds no fault; limits pause rather than falsely complete work.
